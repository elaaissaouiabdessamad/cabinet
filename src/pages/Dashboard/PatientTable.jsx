...
import { FaTrashAlt, FaEye, FaEdit, FaArchive } from "react-icons/fa";
...
const PatientTable = () => {
  ...
  const handleArchiveDossier = async (dossierId) => {
    try {
      await MedicalService.archiveMedicalDossier(dossierId);
      setUpdateTrigger((prev) => !prev);
      toast.success("Dossier archivé avec succès.");
    } catch (error) {
      console.error("Erreur lors de l'archivage du dossier médical:", error);
      toast.error("Erreur lors de l'archivage du dossier médical.");
    }
  };
                ...
                <tr key={patient.id}>
                  ...
                  <td className="py-2 px-4 border text-center">
                    <div className="space-x-2">
                      ...
                      <button
                        onClick={() =>
                          handleArchiveDossier(patient.medicalDossier.id)
                        }
                        className="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600 transition duration-200"
                      >
                        <FaArchive className="inline-block mr-1" /> Archiver
                      </button>
                    </div>
                  </td>
                </tr>
